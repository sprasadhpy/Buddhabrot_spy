""" 
this WOE class has additonal functionality to enforce monotonicity
"""
import operator
import logging
import pandas as pd
...
# overriding WOE class loaded from helper function notebook above.
# basically only clubing special values in the cust auto scores together.
# ONLY used in STEP 1 large model construction.
# in the final model each score variables in treated separately.
# see for example 'fico_woe" below 
class woeimpute(BaseEstimator, TransformerMixin):
    def __init__(self, columns=None, target=None):
        #self.spec_val=spec_val
        self.columns=columns
        self.target='ODEFIND_30JUN2016'
        self.encoders={}
        self.woe=None
    def fit(self, data, y=None):
        if self.columns is None:
            self.columns = woevars
        # Fit a label encoder for each column in the data frame  
        for column in self.columns:
            #print column
            if column in ['bankcard_score', 'telecommunications_score', 'auto_score', 
                          'short_term_lending_score', 'cust_rv_m1_no', 'cust_rv_m2_tn', 
                         'cust_rv_m4_tk', 'cust_rv_m5_all']:
                self.woe_def=WoE()
                self.woe=WoE(50, 20, spec_values={100:'.A', 200:'.A', 222:'.B'}, v_type='c', t_type='b')
                self.encoders[column]=self.woe.fit(pd.Series(data[column]), pd.Series(data[self.target]))
            elif column in ['FICO_SCR_NB']:
                self.woe_def=WoE()
                self.woe=WoE(50, 20, spec_values={ 101:'.A', 102:'.A', 103:'.A', 104:'.B', 
                                                 199:'.A', 198:'.A' }, v_type='c', t_type='b')
                self.encoders[column]=self.woe.fit(pd.Series(data[column]), pd.Series(data[self.target]))
            elif column in ['CBYR_FICO_SCR_NB']:
                self.woe_def=WoE()
                self.woe=WoE(50, 20, spec_values={ 101:'.A', 102:'.A', 103:'.A', 104:'.B', 
                                                 199:'.A', 198:'.A' }, v_type='c', t_type='b')
                self.encoders[column]=self.woe.fit(pd.Series(data[column]), pd.Series(data[self.target]))                   
    ...
    def transform(self, data):
        logger = logging.getLogger(config.LOG_APPLICATION_NAME)
        output = data.copy()
        for column, encoder in self.encoders.items():
            logger.debug("{}, /n , {}".format(column, encoder))
            output[column] = encoder.transform(pd.Series(data[column]), replace_missing=0)['woe']
        return output
class cleanup_scores(TransformerMixin):
    """Modifying score features to make sure there is no bad input. 
    Scores with bad values are set to missing value. Needed for WOE.
    """
    def fit(self, X):
        return self
    def transform(self, X):
        output=X.copy()
        tmp_feat = list(output.columns)
        if any('cust_rv_m2_tn' in s for s in tmp_feat): 
            found_cust_rv_m2_tn = True
        else: found_cust_rv_m2_tn = False
        if any('cust_rv_m5_all' in s for s in tmp_feat):
            found_cust_rv_m5_all = True
        else: found_cust_rv_m5_all = False
        if any('CBYR_FICO_SCR_NB' in s for s in tmp_feat):
            found_CBYR_FICO_SCR_NB = True
        else: found_CBYR_FICO_SCR_NB = False
        if any('FICO_SCR_NB' in s for s in tmp_feat):
            found_FICO_SCR_NB = True
        else: found_FICO_SCR_NB = False
        # set to missing 101, 102, 103 and 199 special codes in cbyr_fico_scr
        if found_CBYR_FICO_SCR_NB: output.loc[(output['CBYR_FICO_SCR_NB']<200) & (~output['CBYR_FICO_SCR_NB'].isin([104, 198])) | (output['CBYR_FICO_SCR_NB']>900), 'CBYR_FICO_SCR_NB']=np.nan
        # same for fico score. 198 is not valid value. 104 needs special treatment.
        if found_FICO_SCR_NB: output.loc[(output['FICO_SCR_NB']<200) & (~output['FICO_SCR_NB'].isin([104])) | (output['FICO_SCR_NB']>619), 'FICO_SCR_NB']=np.nan
        if found_cust_rv_m5_all: output.loc[(output['cust_rv_m5_all']<222) & (~output['cust_rv_m5_all'].isin([100])), 'cust_rv_m5_all']=np.nan
        if found_cust_rv_m2_tn: output.loc[(output['cust_rv_m2_tn']<222) & (~output['cust_rv_m2_tn'].isin([100])), 'cust_rv_m2_tn']=np.nan      
        return output  
class fico_woe(BaseEstimator, TransformerMixin):
    def __init__(self, columns=None, target=None):       
        self.columns=columns
        self.target='ODEFIND_30JUN2016'
        self.encoders={}
    ...
    def fit(self, data, y=None):
        ...
        # Fit a label encoder for each column in the data frame  
        #for column in self.columns:
            #if column in ['FICO_SCR_NB']:
        self.woe_def=WoE()
        self.woe=WoE(50, 20, spec_values={101:'.A', 102:'.A', 103:'.A', 199:'.A', 104:'.B'}, v_type='c', t_type='b')
        self.encoders["FICO_SCR_NB"]=self.woe.fit(pd.Series(data['FICO_SCR_NB']), pd.Series(data[self.target]))               
        self.encoders["FICO_SCR_NB"]=self.woe.force_monotonic()
        return self
    def transform(self, data):
        logger = logging.getLogger(config.LOG_APPLICATION_NAME)
        output = data.copy()
        for column, encoder in self.encoders.items():
            logger.debug("{}, /n , {}".format(column, encoder))
            output[column] = encoder.transform(pd.Series(data[column]), replace_missing=-0.5)['woe']
            if column=="FICO_SCR_NB":
                output[column].replace({0.014254675376820458:-0.5}, inplace=True) # 101, 102, 103, 199
                output[column].replace({0.28938278094497893:0}, inplace=True) # 104           
        return output
class cust_rv_m5_all_woe(BaseEstimator, TransformerMixin):
    def __init__(self, columns=None, target=None):       
        self.columns=columns
        self.target='ODEFIND_30JUN2016'
        self.encoders={}
    ...
    def fit(self, data, y=None):
        ...
        # Fit a label encoder for each column in the data frame  
        ##for column in self.columns:
        #if column in ['cust_rv_m5_all']:
        self.woe_def=WoE()
        self.woe=WoE(50, 20, spec_values={100:'.A', 222:'.A'}, 
                             v_type='c', t_type='b')
        self.encoders['cust_rv_m5_all']=self.woe.fit(pd.Series(data['cust_rv_m5_all']), pd.Series(data[self.target]))   
        self.encoders['cust_rv_m5_all']=self.woe.force_monotonic()
        return self
    def transform(self, data):
        logger = logging.getLogger(config.LOG_APPLICATION_NAME)
        output = data.copy()
        for column, encoder in self.encoders.items():
            logger.debug("{},/n, {}".format(column, encoder))
            #output.loc[(output[column]<101) | (output[column]>900), column]=np.nan
            output['cust_rv_m5_all'] = encoder.transform(pd.Series(data['cust_rv_m5_all']), replace_missing=-0.5)['woe']
            #if column=="cust_rv_m5_all":
            output['cust_rv_m5_all'].replace({0.1914023725847751:0}, inplace=True) # 100 and 222
            output['cust_rv_m5_all'].replace({-0.0027254295721064192:-1.3}, inplace=True) # missing np.nan, 101, 102, 103, 199, <100, >900              
        return output
class cust_rv_m2_tn_woe(BaseEstimator, TransformerMixin):
    def __init__(self, columns=None, target=None):       
        self.columns = columns
        self.target = 'ODEFIND_30JUN2016'
        self.encoders = {}
        self.woe = None
        self.woe_monotonic = None
    def fit(self, data, y=None):
        if self.columns is None:
            self.columns = woevars
        # Fit a label encoder for each column in the data frame  
        #for column in self.columns:
        #if column in ['cust_rv_m2_tn']:
        self.woe_def = WoE()
        self.woe = WoE(50, 20, spec_values={100:'.A', 222:'.A'}, v_type='c', t_type='b')
        self.encoders['cust_rv_m2_tn']=self.woe.fit(pd.Series(data['cust_rv_m2_tn']), pd.Series(data[self.target]))   
        self.encoders['cust_rv_m2_tn']=self.woe.force_monotonic()
        return self
    def transform(self, data):
        logger = logging.getLogger(config.LOG_APPLICATION_NAME)
        output = data.copy()
        for column, encoder in self.encoders.items():
            logger.debug("{}, /n , {}".format(column, encoder))
            #output.loc[(output[column]<101) | (output[column]>900), column]=np.nan
            output['cust_rv_m2_tn'] = encoder.transform(pd.Series(data['cust_rv_m2_tn']), replace_missing=-0.5)['woe']
            #if column=="cust_rv_m2_tn":
            output['cust_rv_m2_tn'].replace({0.1914023725847751:0}, inplace=True) # 100 and 222
            output['cust_rv_m2_tn'].replace({-0.0027254295721064192:-1.3}, inplace=True) # missing np.nan, 101, 102, 103, 199, <100, >900              
        return output
class cbyr_fico_woe(BaseEstimator, TransformerMixin):
    def __init__(self, columns=None, target=None):
        self.columns = columns
        self.target = 'ODEFIND_30JUN2016'
        self.encoders = {}
    ...
    def fit(self, data, y=None):
        ...
        # Fit a label encoder for each column in the data frame  
        # for column in self.columns:
        # if column in ['CBYR_FICO_SCR_NB']:
        self.woe_def = WoE()
        self.woe = WoE(50, 20, spec_values={104:'.A', 198:'.A'}, v_type='c', t_type='b')
        self.encoders["CBYR_FICO_SCR_NB"] = self.woe.fit(pd.Series(data["CBYR_FICO_SCR_NB"]), pd.Series(data[self.target]))   
        self.encoders["CBYR_FICO_SCR_NB"] = self.woe.force_monotonic()
        return self
    def transform(self, data):
        logger = logging.getLogger(config.LOG_APPLICATION_NAME)
        output = data.copy()
        for column, encoder in self.encoders.items():
            logger.debug("{}, /n , {}".format(column, encoder))
            #output.loc[(output[column]<101) | (output[column]>900), column]=np.nan
            output["CBYR_FICO_SCR_NB"] = encoder.transform(pd.Series(data["CBYR_FICO_SCR_NB"]), replace_missing=-0.5)['woe']
            #if column=="CBYR_FICO_SCR_NB":
            output["CBYR_FICO_SCR_NB"].replace({-0.090263697299671986:0}, inplace=True) # 104 and 198
            output["CBYR_FICO_SCR_NB"].replace({-0.10678953958772586:-0.5}, inplace=True) # missing np.nan, 101, 102, 103, 199, <100, >900              
        return output
